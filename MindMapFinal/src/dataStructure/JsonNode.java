...
public class JsonNode {
	...
	public static void readJsonNode(JSONObject object) {
		...
		NodeManager.setHead(new Node(0, name, (int)level));
		Node head = NodeManager.getHead();
		NodeLabel nodeLabel = new NodeLabel(head, null);
		Constants.setComponent(new Point((int)x, (int)y), (int)width, (int)height, nodeLabel);
	...
	private static Node readRecursiveNode(JSONObject object, NodeLabel parent) {
		...
		long width = (long)object.get(WIDTH);
		long height = (long)object.get(HEIGHT);
		Node child = new Node(name, (int)level);
